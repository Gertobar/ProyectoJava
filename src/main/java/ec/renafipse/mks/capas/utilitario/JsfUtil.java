...
public class JsfUtil {
    ...
    public static String getNombreCompletoComponente(FacesContext context, String id) {
        UIViewRoot root = context.getViewRoot();
        //final String componentId = id;
        UIComponent c = encontrarComponente(root, id);
        return c.getClientId(context);
    }

    public static UIComponent encontrarComponente(UIComponent c, String id) {
        if (id.equals(c.getId())) {
            return c;
        }
        Iterator<UIComponent> kids = c.getFacetsAndChildren();
        while (kids.hasNext()) {
            UIComponent found = encontrarComponente(kids.next(), id);
            if (found != null) {
                return found;
            }
        }
