...
@ManagedBean(name = "reclasificacionCarteraBean")
@ViewScoped
public class ReclasificacionCarteraBean extends AbstractController<Comprobante> implements Serializable {
    ...
    public void imprime(Long codIfip,Date fechaCorte, String extension, String tipo) throws JRException, IOException, ClassNotFoundException {
        ...
        String ruta=externalContext.getRealPath("/reporteria/financiero/reclasificacionCartera/reporte/"+nombreReporte+".jasper");
        ruta=ruta.substring(0, ruta.lastIndexOf(nombreReporte));
        getGeneraReporte().getParametros().put("SUBREPORT_DIR", ruta);

        getGeneraReporte().exporta("/reporteria/financiero/reclasificacionCartera/reporte/", nombreReporte,
